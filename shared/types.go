...
// SystemConfig holds the Vault connection settings needed for operations that
// target the Vault system API rather than a specific secret engine mount —
// for example sys/policies, sys/mounts, and auth/token. Unlike Config, it has
// no MountPath field, since none of those endpoints are mount-scoped.
//
// admin.AdminConfig, policies.Config, tokens.Config, and sys.Config are all
// type aliases for SystemConfig. This keeps one environment-resolution
// implementation (token, address, namespace, TLS settings) shared across
// every non-mount-scoped subpackage instead of reimplementing it once per
// subpackage.
type SystemConfig struct {
	...
	// TLSSkipVerify disables TLS certificate verification. It exists for
	// compatibility with Vault's VAULT_SKIP_VERIFY behavior, but it should not
	// be used in production unless there is no sane alternative.
	TLSSkipVerify bool `json:"tls_skip_verify,omitempty"`
